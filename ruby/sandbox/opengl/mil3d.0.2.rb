...
class Gears
  ...
  def draw
    ...
    GL.PushMatrix()
    #GL.Scale(0.5, 0.5, 5.0);
    GL.Translate(@move_x, @move_y, @view_rotb)
    GL.Rotate(@view_rotx, 1.0, 0.0, 0.0)
    GL.Rotate(@view_roty, 0.0, 1.0, 0.0)
    GL.Rotate(@view_rotz, 0.0, 1.0, 1.0)

    GL.PushMatrix();
      GL.Scale(2.0, 0.1, 1.0);
  ...
  def idle
    ...
    @t0_idle = t
    sleep 0.025 #slow down to still get 34 frames but hold cpu cool
    GLUT.PostRedisplay()
  end
  ...
  def reshape(width, height)
    ...
    GL.LoadIdentity()
    GL.Frustum(-1.0, 1.0, -h, h, 5.0, 400.0)
  ...
  def init
    @angle = 0.0
    @view_rotx, @view_roty, @view_rotz,@view_rotb,@move_x,@move_y = 20.0, 30.0, 0.0,0.0,0.0,0.0
