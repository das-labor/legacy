...
#define mirrorcount_v 8	//mirrors on v disc
...
volatile unsigned int target_rpm_v=0;	//send this per i2c
volatile unsigned int rpm_v=0;		//request this per i2c
volatile unsigned int rpm_v_cnt=0;


volatile unsigned char timecounter=0;

volatile unsigned char laserON=0; 
//reduce stretching
...
//	MCUCR |= _BV(SE); // Enable "sleep" mode (low power when idle)
	...
	//******************
	//Port B all inputs only PortB pin 3 pwm output
	//DDRB=(1<<3);
	PORTB = 0x00;	//Internal Pullups off
...
void hsync(){			//called by hardware, laser is in position now
...
void soft_vsync(){		//generated by software after line change
