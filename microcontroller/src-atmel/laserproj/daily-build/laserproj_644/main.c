...
static void hw_init(void)
{
	...
	PRR &= ~ _BV(PRSPI); // SPI powersave off
	SPCR = (1<<SPE)|(1<<MSTR)|(1<<SPR1)|(1<<SPR0); //SPI enable,master mode,speed at 1/64
	...
	//PortB
	//******************
	DDRB |= _BV(PB0)| _BV(PB1)| _BV(PB4) | _BV(PB5) | _BV(PB7); //Pin 0&1 ,MOSI,SCK,SS Output
	PORTB = _BV(PB0)| _BV(PB1)| _BV(PB4) | _BV(PB5) | _BV(PB7); //Pullups on
...
static void soft_vsync(){		//generated by software after line change
	...
	//test first & last 
	  ...
	  array[max_resolution_h/8-1]=(unsigned char) 0x80;
	}
	...
	//test first top, last bottom
	  if(cpixel_v == 1 || cpixel_v == max_resolution_v)
	    for(i=0;i < max_resolution_h/8;i++)
	      array[i]=(unsigned char) 0xFF;
	  else
	    for(i=0;i < max_resolution_h/8;i++)
