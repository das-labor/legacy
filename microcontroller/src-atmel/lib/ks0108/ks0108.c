 ...
 * Modified:	   by Patrick Rudolph mailto:siro@das-labor.org 2.05.10 added transparency 
				   by Patrick Rudolph mailto:siro@das-labor.org 8.05.11 fixed '\n' in strings
				   by Patrick Rudolph mailto:siro@das-labor.org 8.05.11 added '\t' in strings
				   by Patrick Rudolph mailto:siro@das-labor.org 8.05.11 added read busy flag
...
void ks0108Puts(char* str) {
	uint8_t tab_width = ks0108CharWidth(' ')*3;
	while(*str != 0) {
		if(*str == '\n') {
			ks0108GotoXY(0, ks0108Coord.y+ks0108FontRead(ks0108Font+FONT_HEIGHT));
		} else if(*str == '\t') {
			ks0108GotoXY(ks0108Coord.x + tab_width - ks0108Coord.x%(tab_width), ks0108Coord.y);
		} else {
			ks0108PutChar(*str);
		}
		str++;
...
uint8_t ks0108FontHeight(void){
return ks0108FontRead(ks0108Font+FONT_HEIGHT);
}
...
void ks0108GotoX(uint8_t x) {
	ks0108GotoXY(x,ks0108Coord.y);
}
...
void ks0108WriteData(uint8_t data) {
	...
	yOffset = (ks0108Coord.y&0x07);		//same as %8
	if(yOffset) {
		// first page
