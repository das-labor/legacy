 ...
 *****************************/
tetris_playfield_t* tetris_playfield_construct(uint8_t nWidth,
                                               uint8_t nHeight)
{
    if (nWidth <= 16)
    {
...
uint8_t tetris_playfield_removeCompleteLines(tetris_playfield_t* pPl)
{
	...
	if (pPl->status == TETRIS_PFS_DOCKED)
	{
		int y;
		int nSlideDown = 0;
		uint8_t nStartRow = (pPl->nRow < 0) ? 0 : pPl->nRow;
		uint8_t nStopRow =
			((pPl->nRow + 4) >= pPl->nHeight) ? pPl->nHeight - 1 : pPl->nRow + 4;
		uint16_t nFullRow = 0xFFFF >> (16 - pPl->nWidth);
		int i;
		for (i = nStopRow; i >= nStartRow; --i)
		{
