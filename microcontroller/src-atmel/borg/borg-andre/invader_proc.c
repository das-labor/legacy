...
void
procCannon (Cannon * cn, uPixel * shot)
{
...
unsigned char
areAtBorder (Invaders * iv)
{
  int y;
  for (y = SPACESHIP_LINE + 1; y <= GUARD_LINE; ++y)
    {
      if (getInvaderPixel (iv, LEFT_BORDER, y)
	  || getInvaderPixel (iv, RIGHT_BORDER, y))
	{
	  return 1;
	}
    }
...
void
procInvaders (Invaders * iv, uPixel st[MAX_SHOTS])
{
  static unsigned char mv = 0;
...
void
procShots (Invaders * iv,
	   Player * pl,
	   Cannon * cn,
	   Spaceship * sc,
	   unsigned char guards[BORG_WIDTH],
	   uPixel st[MAX_SHOTS], uPixel * shot)
{
