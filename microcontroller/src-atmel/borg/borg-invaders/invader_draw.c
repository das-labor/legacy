...
unsigned char getInvaderPixel(Invaders* iv, unsigned char x, unsigned char y) {
	if( ((x - iv->pos.x) < MAX_INVADER_WIDTH ) 
				&& ((x - iv->pos.x) >= 0 )
				&& ((y -iv->pos.y) < MAX_INVADER_HEIGHT)
				&& ((y -iv->pos.y) >= 0) ) {
...
void setInvaderPixel(Invaders* iv, unsigned char x, unsigned char y, unsigned char val) {
	if( ((x - iv->pos.x) < MAX_INVADER_WIDTH ) 
				&& ((x - iv->pos.x) >= 0 )
				&& ((y -iv->pos.y) < MAX_INVADER_HEIGHT)
				&& ((y -iv->pos.y) >= 0) ) {
...
unsigned char getGuardPixel(unsigned char guards[BORG_WIDTH], 
	...
	if(x < BORG_WIDTH && y == GUARD_LINE)  return guards[x];
	return 0;
}
...
void setGuardPixel(	unsigned char guards[BORG_WIDTH],
							...
							unsigned char val) 
{
		if(x < BORG_WIDTH && y == GUARD_LINE && val <= 3) guards[x] = val;
}
						...
						uPixel* shot)
{
	...
	for(y = 0; y < MAX_INVADER_HEIGHT; y++) {
		for(x = 0; x < MAX_INVADER_WIDTH; x++) {
			...
			if(iv->map[x][y] == 0) continue;
			if(x + iv->pos.x > RIGHT_BORDER) continue;
			if(x + iv->pos.x < 0) continue;
