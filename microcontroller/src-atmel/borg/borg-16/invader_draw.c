...
unsigned char
getInvaderPixel (Invaders * iv, unsigned char x, unsigned char y)
{
...
void
setInvaderPixel (Invaders * iv, unsigned char x, unsigned char y,
		 unsigned char val)
{
  if (((x - iv->pos.x) < MAX_INVADER_WIDTH)
      && ((x - iv->pos.x) >= 0)
...
unsigned char
getGuardPixel (unsigned char guards[BORG_WIDTH],
...
void
setGuardPixel (unsigned char guards[BORG_WIDTH],
