...
void bcan_init()
{
	...
	pdo_message *msg = (pdo_message *) &can_tx_msg;
	msg->addr_src = myaddr;
	msg->addr_dst = 0xff;
...
void process_borg_msg(pdo_message *msg)
{
	...
	case FKT_BORG_MODE:
		...
		borg_can_setmode = msg->data[0];
		break;
	case FKT_BORG_SCROLLTEXT_RESET:
		j = 0;
	case FKT_BORG_SCROLLTEXT_APPEND:
		if(j + msg->dlc < SCROLLTEXT_STRING_SIZE-1){
			memcpy(&scrolltext_text[j], msg->data, msg->dlc-1);
			j += msg->dlc-1;
			scrolltext_text[j] = 0;
		}
		break;
