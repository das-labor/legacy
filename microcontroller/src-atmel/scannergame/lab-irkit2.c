...
//bitCode must be > 0
//0 means an error has occured
uint8_t ir_genCode(uint8_t channel, uint8_t oneCode, uint8_t oneCode_length, uint8_t zeroCode,uint8_t zeroCode_length, uint32_t bitCode, uint8_t codeLen)
...
void ir_sendCode(uint8_t codeLen)
{
	...
	TCCR0=_BV(CS01)|_CS(CS00); //enable Timer0, prescaler 1:64
}
