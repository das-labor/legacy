 ...
 * This Revision: $Id: set-led.c 604 2008-05-13 15:52:47Z cs $
 */
	...
	*/
	if (strcasecmp (argv[1], "status") == 0)
	{
		cnt =
			...
			CUSTOM_RQ_GET_STATUS, 0, 0, buffer, sizeof (buffer),
			5000);
		if (cnt < 1)
		{
			if (cnt < 0)
	...
	} else if ((isOn = (strcasecmp (argv[1], "on") == 0)) || strcasecmp (argv[1], "off") == 0)
	{
		cnt =
			...
			USB_ENDPOINT_OUT, CUSTOM_RQ_SET_STATUS, isOn, 0,
			buffer, 0, 5000);
		if (cnt < 0)
		{
			fprintf (stderr, "USB error: %s\n", usb_strerror ());
	...
	} else if (strcasecmp (argv[1], "test") == 0)
	{
		...
		for (i = 0; i < 50000; i++)
		{
			...
			cnt =
				usb_control_msg (handle,
				USB_TYPE_VENDOR | USB_RECIP_DEVICE |
				USB_ENDPOINT_IN, CUSTOM_RQ_ECHO, value, index,
			...
			rxValue =
				((int) buffer[0] & 0xff) | (((int) buffer[1] & 0xff) << 8);
			rxIndex =
				((int) buffer[2] & 0xff) | (((int) buffer[3] & 0xff) << 8);
