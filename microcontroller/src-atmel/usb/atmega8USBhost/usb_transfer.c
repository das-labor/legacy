...
uint8_t i = 0;
  ...
  if(direction == TRANSMIT)
    {
      DPID=DATA1;
      TokenPacket= TockenPacket(OUT,addr,endp);
 ...
 elseif(direction == RECEIVE)
    {
      TokenPacket= TockenPacket(IN,addr,endp);
      DataPacket=DataPacket(DPID,0);
	...
	if(retries == 0)
	  error = 1;     //Error to many retries needed   
	if(DataPacket->payload_size > 0)
	  memcpy(data, DataPacket->data,DataPacket->payload_size);
	*data_length+=DataPacket->payload_size;
      }while((error == 0) && (DataPacket->payload_size > 0))    
    }
 else
    {
