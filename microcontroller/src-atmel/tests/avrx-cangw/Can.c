...
// ******************** Global Variables ***************************************
static Mutex canTxEmpty, canRxData;
...
void message_fetch(CanMessage *msg){
	unsigned char x;
...
void CanInit(){
	...
	AvrXResetSemaphore(&canRxData);
	AvrXSetSemaphore(&canTxEmpty);
...
{
	if (AvrXTestSemaphore(&canRxData) == SEM_DONE) {
		message_fetch(msg);
		mcp_bitmod( CANINTE, (1<<RX0IE), 0xff); //interrupt back on
		return 0;
	} else 
