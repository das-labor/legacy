...
string getPasswordVerified(string myOutputText)
{
    ...
    for (int i=KEYRONA_MAX_PASSWORD_TRIES; i>0; i--)
    {
...
bool loginSuperUser()
{
    ...
    debug << "KeyronaLogin: My userID = " << myUID << endl;

    cout << "The function you requested requires superuser authentication." << endl;
    if (myUID != 0)
        throw InvalidUID("Keyrona is not executed with UID 0. Please restart as user root.");

    if (! KEYRONA_ALWAYS_REQUIRE_LOGIN)
        return true;
...
KeyronaSubject* getCurrentAdmin(KeyronaStorage &mySubjectStorage)
{
    ...
    if (!KeyronaFindAdmin(mySubjectStorage, existingAdmin))
        throw UnknownSubject("Subject '" + existingAdmin + "' is not an admin.");

    KeyronaSubject *myExistingAdmin = new KeyronaSubject(existingAdmin, mySubjectStorage);
...
bool loginAdmin(KeyronaStorage &mySubjectStorage)
{
    ...
    // load admin
    KeyronaSubject *myAdmin = getCurrentAdmin(mySubjectStorage);
