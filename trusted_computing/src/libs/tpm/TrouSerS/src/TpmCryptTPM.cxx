...
{
    debug << "TpmCryptTPM[TrouSerS]|Constructor(): called!" << endl;
    ...
    if (Tspi_Context_Create(&hContext) != TSS_SUCCESS)
        throw ContextError("TpmCryptTPM[TrouSerS]|Constructor(): Could not create context!");

    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|Constructor(): Could not connect to TPM! The function you requested requires TPM access.\nPlease make sure, that:\n\t- you have a TPM\n\t- your TPM device driver is loaded\n\t- your TPM SRK passphrase is set to the WELL_KNOWN_SECRET\n\t- TrouSerS 'tcsd' is running");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        ...
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|Constructor(): Could not connect to TPM! The function you requested requires TPM access.\nPlease make sure, that:\n\t- you have a TPM\n\t- your TPM device driver is loaded\n\t- your TPM SRK passphrase is set to the WELL_KNOWN_SECRET\n\t- TrouSerS 'tcsd' is running");
    }
    ...
    if ( Tspi_TPM_GetCapability( hTPM, TSS_TPMCAP_PROPERTY, sizeof(UINT32), (BYTE*) &subCap, &len, &owner ) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|Constructor(): Error reading TPM capabilities.");
    }
    ...
    else
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw InvalidTPM("TpmCryptTPM[TrouSerS]|Constructor(): You don't have an owned TPM! Make sure to take ownership of the TPM and set the SRK password to the WELL_KNOWN_SECRET!");
    }
};
...
vector<ByteVector> TpmCryptTPM::seal(ByteVector &dataToSeal, int &local, vector<int> &pcr)
{
    ...
    if (Tspi_Context_Create(&hContext) != TSS_SUCCESS)
        throw ContextError("TpmCryptTPM[TrouSerS]|seal(): Could not create context!");

    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|seal(): Could not connect! Is TrouSerS 'tcsd' running?");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|seal(): Could not connect to TPM!");
    }
    ...
    if (Tspi_Context_CreateObject( hContext, TSS_OBJECT_TYPE_ENCDATA, TSS_ENCDATA_SEAL, &hEncData ) != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Error creating data object.");
    }
    ...
    if (Tspi_Context_LoadKeyByUUID( hContext, TSS_PS_TYPE_SYSTEM, SRK_UUID, &hSRK ) != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Error loading SRK key object.");
    }
	...
	for( pcr_index = 0; pcr_index < pcr.size(); pcr_index++) {
		...
		if (Tspi_PcrComposite_SetPcrValue (hPCR , pcr[pcr_index], pcrvaluelength, pcrvalue ) != TSS_SUCCESS )
		{
			Tspi_Context_FreeMemory(hContext, NULL);
			Tspi_Context_Close(hContext);
			throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Error can't set PCR value");
		}
		pcrvaluelength=NULL;
		pcrvalue=NULL;
    ...
    if ( error0 != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Error sealing data. (" + getTSSError(error0) + ")");
    }
    ...
    if ( Tspi_GetAttribData(hEncData, TSS_TSPATTRIB_ENCDATA_BLOB, TSS_TSPATTRIB_ENCDATABLOB_BLOB, &encLen, &encData) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Error retrieving sealed data.");
    }

    debug << "TpmCryptTPM[TrouSerS]|seal(): Successfully sealed data." << endl;
    ...
    if (plain != dataToSeal)
        throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Unsealed data doesn't match sealed data.");

    return myData;
    
};

ByteVector TpmCryptTPM::unseal(vector<ByteVector> &dataToUnseal)
{
    ...
    if (Tspi_Context_Create(&hContext) != TSS_SUCCESS)
        throw ContextError("TpmCryptTPM[TrouSerS]|unseal(): Could not create context!");

    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|unseal(): Could not connect! Is TrouSerS 'tcsd' running?");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|unseal(): Could not connect to TPM!");
    }
    ...
    if (Tspi_Context_CreateObject( hContext, TSS_OBJECT_TYPE_ENCDATA, TSS_ENCDATA_SEAL, &hEncData ) != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unseal(): Error creating data object.");
    }
    ...
    if ( Tspi_SetAttribData(hEncData, TSS_TSPATTRIB_ENCDATA_BLOB, TSS_TSPATTRIB_ENCDATABLOB_BLOB, sealedData.size(), (BYTE*) sealedData.toCArray()) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unseal(): Error creating sealed data object.");
    }
    ...
    if (Tspi_Context_LoadKeyByUUID( hContext, TSS_PS_TYPE_SYSTEM, SRK_UUID, &hSRK ) != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unseal(): Error loading SRK key object.");
    }
    ...
    if ( Tspi_GetPolicyObject(hSRK, TSS_POLICY_USAGE, &hPolicy) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unseal(): Error creating policy object!");
    }
    ...
    if (Tspi_Policy_SetSecret(hPolicy, TSS_SECRET_MODE_SHA1, TPM_SHA1_160_HASH_LEN, well_known_secret) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unseal(): Error setting SRK secret!");
    }
    ...
    if ( error0 != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unseal(): Error unsealing data. (" + getTSSError(error0) + ")");
    }

    debug << "TpmCryptTPM[TrouSerS]|unseal(): Successfully unsealed data." << endl;
...
ByteVector TpmCryptTPM::bind(ByteVector &dataToBind, UInt32 &bindkeynum)
{
	TSS_HCONTEXT hContext;
	TSS_HTPM	 hTPM;
    TSS_HENCDATA hEncData;
    ...
    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|bind(): Could not connect! Is TrouSerS 'tcsd' running?");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|bind(): Could not connect to TPM!");
    }
    ...
    if (Tspi_Context_CreateObject( hContext, TSS_OBJECT_TYPE_ENCDATA, TSS_ENCDATA_BIND, &hEncData ) != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|bind(): Error creating data object.");
    }      
    // Load SRK by UUID
    if (Tspi_Context_LoadKeyByUUID( hContext, TSS_PS_TYPE_SYSTEM, SRK_UUID, &hSRK ) != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|bind(): Error loading SRK key object.");
    }
	// Get Key by UUID
	...
	if( error0 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|bind(): Error get Key by UUID. (" + getTSSError(error0) + ")");
    }
	// Load Key by UUID
	...
	if( error1 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|bind(): Error load Key by UUID. (" + getTSSError(error1) + ")");
    }
	// Data Bind 
    ...
    if ( error2 != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|bind(): Error binding data. (" + getTSSError(error2) + ")");
    }
    ...
    if ( Tspi_GetAttribData(hEncData, TSS_TSPATTRIB_ENCDATA_BLOB, TSS_TSPATTRIB_ENCDATABLOB_BLOB, &encLen, &encData) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|bind(): Error retrieving binded data.");
    }
    ...
    if (Tspi_Key_UnloadKey(hBindKey) != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|bind(): Error unload Bind Key.");
    }
    
    debug << "TpmCryptTPM[TrouSerS]|bind(): Successfully binded data." << endl;
...
ByteVector TpmCryptTPM::unbind(ByteVector &dataToUnbind, UInt32 &bindkeynum, string &password)
{
    ...
    if (Tspi_Context_Create(&hContext) != TSS_SUCCESS)
        throw ContextError("TpmCryptTPM[TrouSerS]|unbind(): Could not create context!");

    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|unbind(): Could not connect! Is TrouSerS 'tcsd' running?");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|unbind(): Could not connect to TPM!");
    }
    ...
    if (Tspi_Context_CreateObject( hContext, TSS_OBJECT_TYPE_ENCDATA, TSS_ENCDATA_BIND, &hEncData ) != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error creating data object.");
    }
    // Load SRK
    if (Tspi_Context_LoadKeyByUUID( hContext, TSS_PS_TYPE_SYSTEM, SRK_UUID, &hSRK ) != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error loading SRK key object.");
    }
    ...
    if ( Tspi_GetPolicyObject(hSRK, TSS_POLICY_USAGE, &hPolicy) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error creating policy object!");
    }
	// Set SRK secret
    if (Tspi_Policy_SetSecret(hPolicy, TSS_SECRET_MODE_SHA1, TPM_SHA1_160_HASH_LEN, well_known_secret) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error setting SRK secret!");
    }
    ...
    if ( Tspi_SetAttribData(hEncData, TSS_TSPATTRIB_ENCDATA_BLOB, TSS_TSPATTRIB_ENCDATABLOB_BLOB, dataToUnbind.size(), (BYTE*) dataToUnbind.toCArray()) != TSS_SUCCESS)
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error creating binded data object.");
    }
	// Get Key by UUID
    TSS_RESULT error0 = Tspi_Context_GetKeyByUUID( hContext, TSS_PS_TYPE_SYSTEM, hBindKey_UUID, &hBindKey);
	if( error0 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error get Key by UUID. (" + getTSSError(error0) + ")");
    }
    ...
    if (Tspi_GetPolicyObject(hBindKey, TSS_POLICY_USAGE, &hBindKeyPolicy) != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error loading policy object.");
    }
    ...
    if (Tspi_Policy_SetSecret( hBindKeyPolicy, TSS_SECRET_MODE_PLAIN, password.length(), (BYTE*)password.c_str()  ) != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error set policy secret.");
    }
	// Load Key
    TSS_RESULT error1 = Tspi_Key_LoadKey(hBindKey, hSRK);
	if( error1 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error loading Key. (" + getTSSError(error1) + ")");
    }
	// Data Unbind
    ...
    if ( error2 != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error unbind data. (" + getTSSError(error2) + ")");
    }
    ...
    if (Tspi_Key_UnloadKey(hBindKey) != TSS_SUCCESS )
    {
        ...
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|unbind(): Error unload Key.");
    }

    debug << "TpmCryptTPM[TrouSerS]|unbind(): Successfully unbinded data." << endl;
...
ByteVector TpmCryptTPM::create_key(string &password, UInt32 &keynum, string &type)
{
    TSS_HCONTEXT hContext;
	TSS_HTPM	 hTPM;
    TSS_HKEY	 hKey;
    ...
    if (Tspi_Context_Create(&hContext) != TSS_SUCCESS)
        throw ContextError("TpmCryptTPM[TrouSerS]|create_key(): Could not create context!");

    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|create_key(): Could not connect! Is TrouSerS 'tcsd' running?");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|create_key(): Could not connect to TPM!");
    }
	...
	if( error0 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error load Key by UUID. (" + getTSSError(error0) + ")");
    }
    ...
    if ( Tspi_GetPolicyObject(hSRK, TSS_POLICY_USAGE, &hPolicy) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error creating policy object!");
    }
	...
	if( error1 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error set policy secret. (" + getTSSError(error1) + ")");
    }
    ...
    if (Tspi_Context_CreateObject( hContext, TSS_OBJECT_TYPE_RSAKEY, flags, &hKey ) != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
       throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error create Key object.");
    }
	...
	if (Tspi_SetAttribUint32(hKey, TSS_TSPATTRIB_KEY_INFO, TSS_TSPATTRIB_KEYINFO_ENCSCHEME, TSS_ES_RSAESOAEP_SHA1_MGF1) != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error set attributes.");
    }
    ...
    if (Tspi_Context_CreateObject( hContext, TSS_OBJECT_TYPE_POLICY, TSS_POLICY_USAGE, &hKeyPolicy ) != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error create policy object.");
    }
	...
	if (Tspi_Policy_AssignToObject( hKeyPolicy, hKey ) != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error assign policy Object.");
    }
    ...
    if (Tspi_Policy_SetSecret( hKeyPolicy, TSS_SECRET_MODE_PLAIN, password.length(), (BYTE*)password.c_str()  ) != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error set policy secret.");
    }
	...
	if( error2 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error create key. (" + getTSSError(error2) + ")");
    }
    ...
    if( error4 != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|create_key(): Error register key. (" + getTSSError(error4) + ")");
    }
...
void TpmCryptTPM::remove_all_keys_by_uuid()
{
	TSS_HCONTEXT hContext;
	TSS_HTPM	 hTPM;
	UINT32 		 keyblobsize;
	...
	if (Tspi_Context_Create(&hContext) != TSS_SUCCESS)
		throw ContextError("TpmCryptTPM[TrouSerS]|get_keys_by_uuid(): Could not create context!");

    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|get_keys_by_uuid(): Could not connect! Is TrouSerS 'tcsd' running?");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|get_keys_by_uuid(): Could not connect to TPM!");
    }
	...
	if ( error0 != TSS_SUCCESS )
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|get_keys_by_uuid(): Error get registered keys by UUID. (" + getTSSError(error0) + ")");
    }
...
void TpmCryptTPM::change_key_auth(string &password, string &password_old, UInt32 &keynum)
{
	TSS_HCONTEXT hContext;
	TSS_HTPM	 hTPM;
    TSS_HKEY	 hKey;
    ...
    if (Tspi_Context_CreateObject( hContext, TSS_OBJECT_TYPE_POLICY, TSS_POLICY_USAGE, &hKeyPolicy ) != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Error create AIK policy object.");
    }
	...
	if( boo != TSS_SUCCESS )
	{
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TSSError("TpmCryptTPM[TrouSerS]|seal(): Error binding data. (" + getTSSError(boo) + ")");
    }
...
void TpmCryptTPM::delete_key(UInt32 &keynum)
{
...
ByteVector TpmCryptTPM::getRandom(UInt8 amountOfRandomBytes)
{
    if (! amountOfRandomBytes)
        throw TSSError("TpmCryptTPM[TrouSerS]|getRandom(): Invalid amount of bytes requested!");
    ...
    if (Tspi_Context_Create(&hContext) != TSS_SUCCESS)
        throw ContextError("TpmCryptTPM[TrouSerS]|getRandom(): Could not create context!");

    if (Tspi_Context_Connect(hContext, NULL) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw NoTCSD("TpmCryptTPM[TrouSerS]|getRandom(): Could not connect! Is TrouSerS 'tcsd' running?");
    }
    ...
    if (Tspi_Context_GetTpmObject(hContext, &hTPM) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|getRandom(): Could not connect to TPM!");
    }
    ...
    if (Tspi_TPM_GetRandom( hTPM, amountOfRandomBytes, &random ) != TSS_SUCCESS)
    {
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
        throw TPMConnectError("TpmCryptTPM[TrouSerS]|getRandom(): Could not retrieve random numbers from TPM!");
    }

    debug << "TpmCryptTPM[TrouSerS]|getRandom(): Successfully received random numbers." << endl;
