...
void MountHelper::sendMessage(string tosend)
{
    ...
    while(!quit){
        ...
        if (!command.compare(TPMCRYPT_ISC_PASS_MESSAGE))
        {
        ...
        else if (!command.compare(TPMCRYPT_ISC_GET_STRING_MESSAGE))
        {
	     // Get password with threat and timeout //
	     pthread_t thread1;
	     int  iret1,retcode;
        ...
        else if (!command.compare(TPMCRYPT_ISC_MESG_MESSAGE))
        {
            if (!help.empty())
                cout << help << endl;
        }
        else if (!command.compare(TPMCRYPT_ISC_QUIT_MESSAGE))
        {
            if (!help.empty())
                cout << help << endl;
            quit = true;
