...
args=("$@")
echo "$DATESTRING Opening device ${args[0]} and mounting the device to ${args[1]}" >> ${logfile}
echo ${args[2]} | /usr/bin/truecrypt --text --protect-hidden=no -k "" ${args[0]} ${args[1]}  2>> ${logfile} 1>> ${logfile}
check_result $?
