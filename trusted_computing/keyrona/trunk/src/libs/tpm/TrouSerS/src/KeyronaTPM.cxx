...
vector<ByteVector> KeyronaTPM::bind(ByteVector &dataToBind, int &bindkeynum)
{
	TSS_HCONTEXT hContext;
	TSS_HTPM	 hTPM;
    TSS_HENCDATA hEncData;
...
ByteVector KeyronaTPM::unbind(vector<ByteVector> &dataToUnbind, int &bindkeynum, string &password)
{
    ByteVector   bindedData = dataToUnbind[1];
    ...
    if ( Tspi_SetAttribData(hEncData, TSS_TSPATTRIB_ENCDATA_BLOB, TSS_TSPATTRIB_ENCDATABLOB_BLOB, bindedData.size(), (BYTE*) bindedData.toCArray()) != TSS_SUCCESS)
    {
        Tspi_Context_CloseObject(hContext, hEncData);
        Tspi_Context_FreeMemory(hContext, NULL);
        Tspi_Context_Close(hContext);
