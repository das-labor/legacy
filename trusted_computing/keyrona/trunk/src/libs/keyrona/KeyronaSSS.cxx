    ...
    minimumParticipants(n)
{
    ...
    for(int i = 0; i<P; i++ )
    {
        ...
        ByteVector encryptedX = Participants[i]->encryptForSubject(currentX);
        ByteVector encryptedY = Participants[i]->encryptForSubject(currentY);
    ...
    minimumParticipants(n)
{
    ...
    ByteVector encryptedX = myGroup->encryptForGroup(currentX);
    ByteVector encryptedY = myGroup->encryptForGroup(currentY);
...
string KeyronaSSS::retrieveKey(vector<KeyronaSubject*> AvailableParticipants, vector<string> subjectPasswords)
{
    ...
    while ( SubjectIterator != AvailableParticipants.end())
    {
        ...
        decodedX = mySubject->decryptBySubject(decodedX, currentParticipantsPassword);
        decodedY = mySubject->decryptBySubject(decodedY, currentParticipantsPassword);
...
string KeyronaSSS::retrieveKey(vector<KeyronaSubject*> AvailableParticipants, vector<string> subjectPasswords, KeyronaGroup *myGroup, string groupPassword)
{
    ...
    decodedX = myGroup->decryptByGroup(decodedX, groupPassword);
    decodedY = myGroup->decryptByGroup(decodedY, groupPassword);
