 ...
 *
 */
unsigned char pixmap[MAX_Y][MAX_X][COLOR_BYTES];
 ...
 */
void drawLED(float pos_x, float pos_y, float pos_z) {
	glPushMatrix();
...
void display(void){
        ...
        glPushMatrix();
        glTranslatef(MAX_Y*2., 0., MAX_X*2.);
        tbMatrix();
		...
		for (x = 0; x < MAX_X; x++) {
				for (y = 0; y < MAX_Y; y++) {
						...
						glColor4f(pixmap[y][x][1]/255.0, 
									pixmap[y][x][2]/255.0, 
									pixmap[y][x][0]/255.0,
									1);
						drawLED((float)x*4.0, 4.0, (float)(MAX_Y-1-y)*4.0);
				}
		}
...
void reshape(int width, int height)
{
  ...
  gluLookAt(MAX_X*2., MAX_X*2.+50., MAX_Y*2.,
            MAX_X*2., MAX_X*2.,     MAX_Y*2.,
            0.0,      0.0,          1.0); 
...
int main(int argc, char **argv) {
    ...
    win = glutCreateWindow("Nebelborg Simulator");
