...
int main(int argc, char *argv[])
{
	...
	system("export MPD_HOST=10.0.1.2");
                    ...
                    union cwiid_mesg mesg[], struct timespec *timestamp)
{
	...
	for (i=0; i < mesg_count; i++)
	{
		...
		case CWIID_MESG_STATUS:
			...
			break;
		case CWIID_MESG_BTN:
			printf("Button Report: %.4X %i\n", mesg[i].btn_mesg.buttons,mesg[i].btn_mesg.buttons);
			if(mesg[i].btn_mesg.buttons == 4096 )system("mpc volume +10");
			if(mesg[i].btn_mesg.buttons == 16 )system("mpc volume -10");
			if(mesg[i].btn_mesg.buttons == 512 ) seek1=1;
			if(mesg[i].btn_mesg.buttons == 256 ) seek2=1;
			if(mesg[i].btn_mesg.buttons == 128 )system("ssh rl ./speak.sh \\\"welcome to the la bor\\\"");
			if(mesg[i].btn_mesg.buttons == 4 && pause==0 ){system("mpc pause"); pause=1;}
			else if(mesg[i].btn_mesg.buttons == 4 && pause==1 ){system("mpc play"); pause=0;}
			if(mesg[i].btn_mesg.buttons == 0 ){seek1=0; seek2=0;}
			break;
		case CWIID_MESG_ACC:
			printf("Acc Report: x=%d, y=%d (%i), z=%d nakka=%i\n",
                   mesg[i].acc_mesg.acc[CWIID_X],
			       mesg[i].acc_mesg.acc[CWIID_Y],
			       mesg[i].acc_mesg.acc[CWIID_Y],
			       mesg[i].acc_mesg.acc[CWIID_Z],
				prell_nakka);
				if(mesg[i].acc_mesg.acc[CWIID_Y] > 200 && prell_nakka == 0){prell_nakka++; system("mpc next");}
				else if(mesg[i].acc_mesg.acc[CWIID_Y] > 200 && prell_nakka < 10 )prell_nakka++; 
				else if(mesg[i].acc_mesg.acc[CWIID_Y] < 200 && prell_nakka > 0) prell_nakka--;
				if(seek1==1 && takt==0) system("mpc seek +3");
				if(seek2==1 && takt==0) system("mpc seek -3");
				if(takt<5) takt++;
				else takt=0;
			break;
		case CWIID_MESG_IR:
