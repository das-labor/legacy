...
1) Lade die .ics-Dateien fuer alle drei Muellsorten von der USB-Seite herunter und lege sie unter den Dateinamen
USB_Restmuell.ics, USB_Blaumuell.ics (Papier) und USB_Gelbmuell.ics (komb. Wertstofftonne) in diesem Verzeichnis ab. 
(http://www.usb-bochum.de/buerger/Leerungstermine.php)
2) OBSOLET: Gehe auf die REMONDIS-Seite (http://www.remondis.de/dsd-bochum/ - vorsicht die suche ist arsch es muss genau 
...
1) USB und Remondis (wie in 2010)
2) nur USB (wie in 2011)
Momentan ist das Makefile und das Skript icsconverter.rb auf Szenario (2) eingestellt.

Die beiden Skripte (siehe das makefile fuer den Aufruf) parsen die Termine und generieren daraus ein .ics (bzw. bei
"gelbkalender_snippetproducer.rb" einen Abschnitt einer .ics-Datei). Die woechentlich wiederkehrenden Termine in den beiden
input-ics werden in einzelne Termine, verwandelt, alle Termine werden auf den Tag zuvor verlegt als ganztaegige Termine,
und mit einer in den Skripten konfigurierbaren "Summary" versehen.
...
Die Input-Dateien, mit denen die Skripte urspruenglich funktioniert haben, liegen im Verzeichnis Inputs_2010 bzw. Inputs_2011.
