...
#define HASH_BLOCKSIZE BMW_LARGE_BLOCKSIZE_B
...
void hash_file(int fd, uint64_t fsize, void* digest){
	...
	while(main_blocks--){
		...
		bufptr = buffer;
		do{
			bmw_large_nextBlock(&hctx, bufptr);
	...
	bmw_large_lastBlock(&hctx, buffer, last_bytes*8);

	bmw384_ctx2hash(digest, &hctx);
}
...
void hash_stdin(void *digest){
	...
	while(!feof(f)){
		...
		for(i=0; i<read_size-HASH_BLOCKSIZE; i+=HASH_BLOCKSIZE){
			bmw_large_nextBlock(&hctx, buffer+i);
		}
		if(read_size-i){
	...
	fclose(f);
	bmw_large_lastBlock(&hctx, buffer, read_size*8);
	bmw384_ctx2hash(digest, &hctx);
}
