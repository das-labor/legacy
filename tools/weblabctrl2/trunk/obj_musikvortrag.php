...
class c_musikvortrag extends c_content{
  protected $dname;
  protected $actorfile="act_musikvortrag.php";
  protected $updaterfile="updt_musikvortrag.php";
  protected $content;
  protected $divstr;
  protected $cssstr;
  protected $setfile;
  ...
  {
    $this->setobjname("musikvortrag");
    $this->setdisplayname("Musik Vortrag");
    $this->setsetFile("act_musikvortrag.php");
    $this->setsetfilecmd("musikvortrag");
    $this->setid("id_musikvortrag");
  }
  ...
  protected function initContent()
  {
    ...
    $this->content .= "<div name=\"".$this->myid."link\" id=\"".$this->myid."link\" style=\"font-size:small\"> <a href=\"http://wl/vortrag/index.php\">webinterface</a></div>";
  ...
  protected function initCSS()
  {
    ...
    $this->cssstr .= "div.slider div.handle { \n";
#    $this->cssstr .= "  width:10px; \n";
#    $this->cssstr .= "  height:15px; \n";
  ...
  public function getupdaterjs()
  {
    ...
    if( (!isset($localstate["musik_vortrag_last"])) ||
       ($localstate["musik_vortrag_last"]+10 < time()) ||
	(!isset($localstate["musik_vortrag_status"])))
      {
	
	unset($status);
  
	exec("mpc -h jukebox.lan.labor-bochum.net -p 6603 current",$status);
	
	$titel = $status[0];

	$localstate["musik_vortrag_title"] = $status[0];

	unset($status);

	exec("mpc -h jukebox.lan.labor-bochum.net -p 6603 volume",$status);
  
	$volume = sscanf($status[0],"volume: %d");
	
	unset($status);
    
	exec("mpc -h jukebox.lan.labor-bochum.net -p 6603",$status);

	if(strcmp($status[0],$titel)==0) 
	  {
	    $indata = sscanf  ( $status[1] , "%s\t%s\t%d:%d/%d:%d\t(%d" );
	    $localstate["musik_vortrag_status"]=substr($indata[0],1,-1);
	    $localstate["musik_vortrag_current"]=$indata[2].":".$indata[3];
	    $localstate["musik_vortrag_end"]=$indata[4].":".$indata[5];
	    $localstate["musik_vortrag_pos"]=$indata[6];
	  }
	else 
	  {
	    $localstate["musik_vortrag_current"]="0:0";
	    $localstate["musik_vortrag_end"]="0:0";
	    $localstate["musik_vortrag_pos"]=0;
	    $localstate["musik_vortrag_status"]="stop";
	  }
	$localstate["musik_vortrag_volume"]=$volume[0];
	$localstate["musik_vortag_last"]=time();
      }
    ...
    $updaterjs .= $this->myid."_obj_slider_vol.setValue('".$localstate["musik_vortrag_volume"]."');\n";
    $updaterjs .= $this->myid."_obj_slider_seek.setValue('".$localstate["musik_vortrag_pos"]."');\n";

    $updaterjs .= $this->myid."settitle(\"".htmlspecialchars($localstate["musik_vortrag_title"])."\");\n";
    $updaterjs .= $this->myid."setplaypos(\"".htmlspecialchars($localstate["musik_vortrag_current"])."/".htmlspecialchars($localstate["musik_lounge_end"])."\");\n";
