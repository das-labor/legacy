...
if($_GET["doit"])
  {
      ...
      case "play" :
	{
	  if($localstate["musik_bastel_status"]!="playing")
	    {
	      if($control=="enabled") exec("MPD_PORT=6602 MPD_HOST=10.0.1.6 mpc play") ;
      ...
      case "stop":
	{
	  if($localstate["musik_bastel_status"]!="stop")
	    {
	      if($control=="enabled") exec("MPD_PORT=6602 MPD_HOST=10.0.1.6 mpc stop") ;
      ...
      case "pause":
	{
	  if($localstate["musik_bastel_status"]!="paused")
	    {
	      if($control=="enabled") exec("MPD_PORT=6602 MPD_HOST=10.0.1.6 mpc pause") ;
	...
	break;
      case "prev":
	{
	  if($control=="enabled") exec("MPD_PORT=6601 MPD_HOST=10.0.1.6 mpc prev") ;
	}
	break;
      case "next":
	{
	  if($control=="enabled") exec("MPD_PORT=6601 MPD_HOST=10.0.1.6 mpc next") ;
	}
	break;
      case "volume":
	{
	  if ( $localstate["musik_bastel_volume"]!=$value_dec)
	    {
	      if($control=="enabled") exec("MPD_PORT=6602 MPD_HOST=10.0.1.6 mpc volume ".$value_dec) ;
      ...
      case "seek":
	{
	  if ( $localstate["musik_bastel_pos"]!=$value_dec)
	    {
	      if($control=="enabled") exec("MPD_PORT=6602 MPD_HOST=10.0.1.6 mpc seek ".$value_dec."\%") ;
