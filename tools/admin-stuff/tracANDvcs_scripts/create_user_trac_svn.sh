...
TRACSHORT="msp" \ # der Kurzname zu dem Projekt. die ist auch 
                \ # der Verzeichnisname
./create_user_trac_svn.sh


was passiert beim Aufrufen:
* es wird ein trac unter /srv/TRACs/TRACSHORT erstellt
* es wird ein svn unter /srv/SVNs/TRACSHORT erstellt
* es wird im trac der user USERNAME angelegt der Adminrechte hat
* es wird im svn der user USERNAME angelegt der schreibrechte hat
* es wird fuer trac+svn eine apacheconfig angelegt in 
...
danach sollte das svn+trac fuer den benutzer vorhanden sein
...
if [[ -e ${TRACBASEDIR}/${TRACSHORT} ]];then
    echo "sorry das TRAC ${SVNBASEDIR}/${TRACSHORT} gibt es schon"
    exit 1
fi

if [[ -e ${SVNBASEDIR}/${TRACSHORT} ]];then
    echo "sorry das SVN ${SVNBASEDIR}/${TRACSHORT} gibt es schon"
    exit 1
fi
...
mkdir -p ${SVNBASEDIR}
mkdir -p ${TRACBASEDIR}
${SVNADMIN} create ${SVNBASEDIR}/${TRACSHORT}
echo ""
echo "svn done..."
echo ""
echo ""
...
echo ""
${HTPASSWD} -c -b ${SVNBASEDIR}/${TRACSHORT}/conf/htpasswd.labor ${USERNAME} ${PASSWORD}
echo ""
echo "passwort und user erstellt"
echo ""
${TRACADMIN} "${TRACBASEDIR}/${TRACSHORT}" initenv "${TRACNAME}" sqlite:db/trac.db svn "${SVNBASEDIR}/${TRACSHORT}"
echo ""
echo "trac erstellt"
echo ""
...
<Location /usersvn/${TRACSHORT}>
  DAV svn
  SVNPath ${SVNBASEDIR}/${TRACSHORT}
  AuthType Basic
  AuthName "${TRACSHORT} Subversion Repository"
  AuthUserFile ${SVNBASEDIR}/${TRACSHORT}/conf/htpasswd.labor
...
<Location "/trac/login">
        AuthType Basic
        AuthName "${TRACSHORT}r Trac Auth"
        AuthUserFile ${SVNBASEDIR}/${TRACSHORT}/conf/htpasswd.labor
        Require valid-user
</Location>
EOF
...
htdigest_realm = 
password_file = ${SVNBASEDIR}/${TRACSHORT}/conf/htpasswd.labor
password_store = HtPasswdStore


[downloads]
ext = zip,gz,bz2,rar,pdf,ps,tar,rar
path = ${SVNBASEDIR}/${TRACSHORT}/downloads
title = Downloads
...
repository_dir = ${SVNBASEDIR}/${TRACSHORT}
repository_type = svn
timeout = 20
...
chown -R ${APACHEUSER}:${APACHEGROUP} ${SVNBASEDIR}/${TRACSHORT}
...
SVN  :      https://www.das-labor.org/usersrv/${TRACSHORT}
