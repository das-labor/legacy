...
void mqq_load_pgm_key(void* buffer, mqq160_sign_key_t* key, const mqq160_sign_key_t* key_P){
	...
	memcpy(buf_ptr, key->a, MQQ160SIGN_A_SIZE);
	key->a = buf_ptr;
	buf_ptr += MQQ160SIGN_A_SIZE;

	memcpy(buf_ptr, key->cc1, MQQ160SIGN_CC1_SIZE);
	key->cc1 = buf_ptr;
	buf_ptr += MQQ160SIGN_CC1_SIZE;

	memcpy(buf_ptr, key->cc2, MQQ160SIGN_CC2_SIZE);
	key->cc2 = buf_ptr;
	buf_ptr += MQQ160SIGN_CC2_SIZE;

	memcpy(buf_ptr, key->rp1, MQQ160SIGN_RP1_SIZE);
	key->rp1 = buf_ptr;
	buf_ptr += MQQ160SIGN_RP1_SIZE;

	memcpy(buf_ptr, key->rp5, MQQ160SIGN_RP5_SIZE);
	key->rp5 = buf_ptr;
