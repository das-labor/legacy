...
uint8_t dsa_verify_bigint(const dsa_signature_t* s, const bigint_t* m,
	...
	bigint_word_t w_b[ctx->domain.q.length_B], u1_b[ctx->domain.q.length_B*2], u2_b[ctx->domain.q.length_B*2];
	bigint_word_t v1_b[ctx->domain.p.length_B*2], v2_b[ctx->domain.p.length_B];
	w.wordv = w_b;
	u1.wordv = u1_b;
	u2.wordv = u2_b;
	v1.wordv = v1_b;
...
uint8_t dsa_verify_message(const dsa_signature_t* s, const void* m, uint16_t m_len_b,
	...
	bigint_t z;
	uint16_t n_B = ctx->domain.q.length_B;
