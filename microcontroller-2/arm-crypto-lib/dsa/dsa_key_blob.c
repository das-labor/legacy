...
void load_dsa_key_blob(dsa_ctx_t* ctx){
	...
	ctx->priv.length_B=(PRIV_LEN_B+sizeof(bigint_word_t)-1)/sizeof(bigint_word_t);
	ctx->pub.wordv = (bigint_word_t*)((uint8_t*)ctx->priv.wordv+PRIV_LEN_B);
	ctx->pub.length_B = (PUB_LEN_B+sizeof(bigint_word_t)-1)/sizeof(bigint_word_t);
	ctx->domain.p.wordv = (bigint_word_t*)((uint8_t*)ctx->priv.wordv+PRIV_LEN_B+PUB_LEN_B);
	ctx->domain.p.length_B = (P_LEN_B+sizeof(bigint_word_t)-1)/sizeof(bigint_word_t);
	ctx->domain.q.wordv = (bigint_word_t*)((uint8_t*)ctx->priv.wordv+PRIV_LEN_B+PUB_LEN_B+P_LEN_B);
	ctx->domain.q.length_B = (Q_LEN_B+sizeof(bigint_word_t)-1)/sizeof(bigint_word_t);
	ctx->domain.g.wordv = (bigint_word_t*)((uint8_t*)ctx->priv.wordv+PRIV_LEN_B+PUB_LEN_B+P_LEN_B+Q_LEN_B);
	ctx->domain.g.length_B = (G_LEN_B+sizeof(bigint_word_t)-1)/sizeof(bigint_word_t);
