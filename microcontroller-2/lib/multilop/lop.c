		...
		switch(b){
			...
			case LOP_TYPE_STREAMSYNC:
				...
				if(ctx->on_streamsync){
					if(ctx->optctx){
						((void(*)(void*))ctx->on_streamsync)(ctx->optctx);
					} else {
						ctx->on_streamsync();
					}
				}
...
void lop_process_l2(lop_ctx_t* ctx, uint8_t b){
	switch(ctx->rxstate){
		case idle:
			...
			if(ctx->on_streamrx){
				if(ctx->optctx){
					((void(*)(void*,uint8_t))ctx->on_streamrx)(ctx->optctx,b);
				} else {
					ctx->on_streamrx(b);
				}
			}
		...
		case message:
			...
			if(ctx->msgidx==(uint32_t)ctx->msglength+1){ /* end of message */
				if(ctx->on_msgrx){
					((void(*)(void*,uint16_t,void*))ctx->on_msgrx)(ctx->optctx, ctx->msglength, ctx->msgbuffer);
				} else {
