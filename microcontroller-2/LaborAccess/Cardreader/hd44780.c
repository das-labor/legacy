...
//This one is for Datalines 7:4 of display connected to Atmel 3:0
//#define IF 1

//This one is for Datalines 7:4 of display connected to Atmel 7:4
#define IF 2

//Port, to which the Datalines of the Display are connected
#define DISP_DDR_D DDRD
#define DISP_PORT_D PORTD
#define DISP_PIN_D PIND

//Port, to which the Control Lines of the Display are connected
#define DISP_DDR_C DDRB
#define DISP_PORT_C PORTB
...
void hd44780_send(uint8_t data){
...
#elif IF==2
	DISP_DDR_D &= 0X0F;		//Data lines 7:4 to Input
	DISP_PORT_D |= 0xF0;		//Pullups on on Data lines
#else
	...
	do{
		...
		asm volatile ("nop\n\tnop\n\t"::);		
#if IF==1
		busy = (DISP_PIN_D & 0x08);
		DISP_PORT_C &= ~_BV(PIN_E);
		DISP_PORT_C |= _BV(PIN_E);
		DISP_PORT_C &= ~_BV(PIN_E);
#elif IF==2
		busy = (DISP_PIN_D & 0x80);
		DISP_PORT_C &= ~_BV(PIN_E);
		DISP_PORT_C |= _BV(PIN_E);
		DISP_PORT_C &= ~_BV(PIN_E);
#else
...
#elif IF==2
	DISP_DDR_D |= 0xF0;		//Data Lines to Output
#else
	DISP_DDR_D = 0xFF;
#endif
}
...
uint8_t hd44780_read(){
	...
	DISP_PORT_C |= 1<<PIN_E;
	asm volatile ("nop\n\tnop\n\t"::);
	address = DISP_PIN_D & 0x07; //dont read busy flag
	DISP_PORT_C &= ~_BV(PIN_E);
	address <<= 4;
	DISP_PORT_C |= _BV(PIN_E);
	asm volatile ("nop\n\tnop\n\t"::);
	address |= DISP_PIN_D & 0x0F;
	DISP_PORT_C &= ~_BV(PIN_E);
	...
	DISP_PORT_C |= 1<<PIN_E;
	asm volatile ("nop\n\tnop\n\t"::);
	address = DISP_PIN_D & 0x70; //dont read busy flag
	DISP_PORT_C &= ~_BV(PIN_E);
	asm volatile ("nop\n\tnop\n\t"::);
	DISP_PORT_C |= _BV(PIN_E);
	asm volatile ("nop\n\tnop\n\t"::);
	address |= (DISP_PIN_D >> 4);
	DISP_PORT_C &= ~_BV(PIN_E);
	...
	DISP_PORT_C |= 1<<PIN_E;
	asm volatile ("nop\n\tnop\n\t"::);
	address = DISP_PIN_D & 0x7F; //dont read busy flag
	DISP_PORT_C &= ~_BV(PIN_E);
