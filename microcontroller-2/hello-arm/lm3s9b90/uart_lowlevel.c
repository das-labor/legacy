...
#define UART_FREQ 16000000UL

#ifndef UART_FREQ
#error "You must define UART_FREQ to be the UART frequency!"
#endif

void calc_baud_values(uint32_t baudrate, uint16_t* intdivider, uint8_t* fracdivider, uint8_t* highspeed){
	uint32_t tmp;
//	*highspeed = (baudrate>=38400)?1:0;
	*highspeed = 0;
	tmp = UART_FREQ*128/((*highspeed?8:16)*baudrate);
	tmp++;
	tmp>>=1;
