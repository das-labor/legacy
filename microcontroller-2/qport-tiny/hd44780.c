...
uint8_t hd44780_send(uint8_t data){
	uint8_t busy;
	uint16_t timeout=DISPLAY_TIMEOUT;
	
#ifdef IF4BIT	
	DISP_PORT_C |= _BV(PIN_E1);
	DISP_PORT_D &= 0xF0;
...
uint8_t hd44780_command(uint8_t command){
	DISP_PORT_C &= ~(1<<PIN_RS);
	return hd44780_send(command);
}
...
uint8_t hd44780_init(){
	...
	DISP_PORT_D = 0x00;
#endif
	DISP_DDR_C |= (_BV(PIN_E1)|_BV(PIN_E2)|_BV(PIN_RW)|_BV(PIN_RS));
	DISP_PORT_C &= ~(_BV(PIN_E1)|_BV(PIN_E2)|_BV(PIN_RW)|_BV(PIN_RS));
	...
	if(hd44780_command(0x28)) goto timeout;// 2 Lines 5x7 DOT
	...
	if(hd44780_command(0x30)) goto timeout;
	if(hd44780_command(0x38)) goto timeout;// 2 Lines 5x7 DOT
#endif

	if(hd44780_command(0x10)) goto timeout;// cursor move
	if(hd44780_command(0x0F)) goto timeout;// Display on, cursor on, cursor blink

	if(hd44780_command(0x01)) goto timeout;// clear Display
