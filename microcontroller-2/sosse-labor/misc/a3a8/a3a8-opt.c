...
CODE static const uint8_t tableidx[5] = {
	  0>>2,	/* Table 0 */
	512>>2,	/* Table 1 */
	768>>2,	/* Table 2 */
	896>>2,	/* Table 3 */
...
CODE static const uint8_t table[992] = {
	/* Table 0 */
...
static inline uint8_t getbit( uint8_t *array, uint8_t pos )
{
	return ( (*( array + (pos>>3) )) >> (7-(pos&0x07)) ) & 0x01;
}

void A3A8( /* in */ uint8_t key[16], /* in+out */ uint8_t randout[16] )
{
	uint8_t x[32];
	uint8_t i, j, k, l, m, n;
	uint16_t y, z;
