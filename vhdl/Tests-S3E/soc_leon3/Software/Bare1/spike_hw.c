...
gpio_t  *gpio0  =  (gpio_t *)0xF0000000;
uart_t  *uart0  =  (uart_t *)0xF0010000;
timer_t *timer0 = (timer_t *)0xF0020000;
 ...
 * IRQ handling
 */
void irq_setup(int irl, void (*handler)(void) )
{
}
...
void irq_handler(uint32_t irl)
{
	...
	case 12:                                 /* timer0.0 (system tic) */
		tcr = timer0->tcr0;					// clear interrupt flag
		msec++;
		break;
	case 11:                                                 /* scope */
		break;                                            
	case 10:                                              /* uart0 rx */
		...
		if (diff < UART_RXBUFSIZE -1) {      // buffer not full
			*rxhead = uart0->databuf;        // read reg and clear IRQ
			if (++rxhead == (rxbuf + UART_RXBUFSIZE)) rxhead = rxbuf;
		} else {                             // buffer full
			uart0->databuf;                  // read reg to clear IRQ
		}
		break;
	case 6:                                               /* timer0.1 */
		break;
